...
export default function TaskList() {
    ...
    const savedState = localStorage.getItem('taskAmount')
    const [taskAmount, setTasksAmount] = useState(savedState ? JSON.parse(savedState) : 0)


    useEffect(() => {
        if(taskAmount) {
            localStorage.setItem('taskAmount', JSON.stringify(taskAmount))
        }
    }, [taskAmount])
    ...
    return (
        ...
        <header className="header">
            ...
            <div className="header__wrapper">
               <div className="header__title">What are your plans for today?</div>
            </div>
               ...
               <ul id="list" className="tasks__list">
                {
                    data.map(item => {
                        let i = 0
                        return <li key={item.id} id={item.id} className='tasks__item'>

                        <div id={item.id + i} className="tasks__item__tick-wrapper">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="white"  viewBox="0 0 26 26" width="40px" height="40px"><path d="M 22.566406 4.730469 L 20.773438 3.511719 C 20.277344 3.175781 19.597656 3.304688 19.265625 3.796875 L 10.476563 16.757813 L 6.4375 12.71875 C 6.015625 12.296875 5.328125 12.296875 4.90625 12.71875 L 3.371094 14.253906 C 2.949219 14.675781 2.949219 15.363281 3.371094 15.789063 L 9.582031 22 C 9.929688 22.347656 10.476563 22.613281 10.96875 22.613281 C 11.460938 22.613281 11.957031 22.304688 12.277344 21.839844 L 22.855469 6.234375 C 23.191406 5.742188 23.0625 5.066406 22.566406 4.730469 Z"/></svg>
                        </div>
                           ...
                           <div className="tasks__item__buttons">
                               ...
                               <button 
                               onClick={() => {
                                   let tick = document.getElementById(item.id + i)
                                   tick.className = 'tasks__item__tick-wrapper tick'
                               ...
                               onClick={
                                   e => {
                                       e.stopPropagation()
                                       let li = document.getElementById(item.id)
                                       li.className = 'tasks__item remove'
                ...
                <div className="tasks__list__total">
                    <p className="total-tasks">{taskAmount} tasks in total.</p>
                    <button
                    style={taskAmount < 2 ? {display: 'none'} : null}
                    id="wipe"
                    onClick={() => {
                       getData()
                       data.map(task => {
                ...
                className="tasks__list__total__delete">Delete all</button>
                </div>
               </ul>
           </div>
        </main>
